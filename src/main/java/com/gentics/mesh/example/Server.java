...
public class Server extends AbstractVerticle {
	...
	public void start() throws Exception {
				...
				.handler(rc -> {
					...
					resolvePath(path).subscribe(response -> {
						...
						if (response.isBinary()) {
							...
							rc.response().end(response.getDownloadResponse()
										 .getBuffer());
						...
						JsonObject jsonObject = toJsonNode(
						...
						if ("category".equals(schemaName)) {
							Single<JsonObject> childrenObs = loadCategoryChildren(
									response.getNodeResponse());
		...
		router.route()
				.handler(rc -> {
					String file = DEFAULT_TEMPLATE_DIRECTORY + File.separator
							+ rc.get("tmplName");
